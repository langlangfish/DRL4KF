    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                self.logger.record("learn.yaml/learning_rate", self.policy.optimizer.state_dict()['param_groups'][0]['lr'])
        ...
        # Logs
        self.logger.record("learn.yaml/entropy_loss", np.mean(entropy_losses))
        self.logger.record("learn.yaml/policy_gradient_loss", np.mean(pg_losses))
        self.logger.record("learn.yaml/value_loss", np.mean(value_losses))
        self.logger.record("learn.yaml/approx_kl", np.mean(approx_kl_divs))
        self.logger.record("learn.yaml/clip_fraction", np.mean(clip_fractions))
        self.logger.record("learn.yaml/loss", loss.item())
        self.logger.record("learn.yaml/explained_variance", explained_var)
        if hasattr(self.policy, "log_std"):
            self.logger.record("learn.yaml/std", th.exp(self.policy.log_std).mean().item())

        self.logger.record("learn.yaml/n_updates", self._n_updates, exclude="tensorboard")
        self.logger.record("learn.yaml/clip_range", clip_range)
        if self.clip_range_vf is not None:
            self.logger.record("learn.yaml/clip_range_vf", clip_range_vf)
        # added recordings 20221021
